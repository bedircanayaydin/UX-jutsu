            ...
            try:
                async with userge.conversation(chat_id, timeout=8) as conv:
                    response = await conv.get_response(
                        mark_read=True,
                        filters=(
                            filters.user([609517172, 2059887769,1376954911]) & ~filters.service
                        ),
                    )
                    resp = response.text
                    if not (
...
async def fban_p(message: Message):
    ...
    try:
        if reply.from_user:
            bannee = reply.from_user.id
        elif reply.sender_chat:
            bannee = reply.sender_chat.id
        await userge.ban_chat_member(chat_id=reply.chat.id, user_id=bannee)
        await reply.delete()
    except BaseException:
        pass
