...
@userge.on_cmd("cygisk$", about={"header": "Get Latest Cygisk Zip and Manager"})
async def magisk_(message: Message):
    """Get Latest CYGISK"""
    cygisk_repo = "https://cdn.jsdelivr.net/gh/Cygisk/Cygisk-Files@master/configs/"
    cygisk_dict = {
        "⦁ 𝗦𝘁𝗮𝗯𝗹𝗲": cygisk_repo + "stable.json",
        "⦁ 𝗕𝗲𝘁𝗮": cygisk_repo + "beta.json",
        "⦁ 𝗖𝗮𝗻𝗮𝗿𝘆": cygisk_repo + "canary.json",
    }
    releases = "<code>𝗟𝗮𝘁𝗲𝘀𝘁 Cy𝗴𝗶𝘀𝗸 𝗥𝗲𝗹𝗲𝗮𝘀𝗲:</code>\n\n"
    for name, release_url in cygisk_dict.items():
        data = get(release_url).json()
        releases += (
            f'{name}  :   [APK v{data["magisk"]["version"]}]({data["magisk"]["link"]})\n'
        )
